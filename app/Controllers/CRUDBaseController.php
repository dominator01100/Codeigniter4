...
class CRUDBaseController extends BaseController {
    ...
    private function _setFromDBResult() {
        ...
        if ($this->paginated) $this->rows = $this->model->paginate(10);
    ...
    private function buildRelations() {
        if (!count($this->selectsFk)) return;
        $this->selectsData[$this->selectsFk[0]->localField] = new RelationData($this->selectsFk[0]->tableName->findAll(), $this->selectsFk[0]->fkField, $this->selectsFk[0]->fkFieldName);
    ...
    private function _validateListName() {
        if (count($this->listName) == 0) {
            foreach ($this->eheading as $key => $h) $this->listName[$h] = $h;
        }
        if (count($this->listName) !== count($this->eheading)) throw new \Exception("La longitud del atributo listName tiene que coincidir con la cantidad de columnas a mostrar.");
        foreach ($this->eheading as $key => $h) {
            try {
                $this->listName[$h];
            }
